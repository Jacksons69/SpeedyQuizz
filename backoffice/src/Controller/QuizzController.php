...
class QuizzController extends AbstractController
{
    ...
    public function jokerUpdate(UserRepository $userRepo, EntityManagerInterface $em, Request $request, TokenDecoder $tokenDecoder) {
        ...
        $jokerToUpdate = 'get'.str_replace('_', '', ucwords($jokerToUpdate, '_')); // Avec ucwords, on met une majuscule aux mots après les '_', puis on les remplaces avec str_replace par rien pour avoir du camelCase
        $userJoker = call_user_func([$user, $jokerToUpdate]);
        
        if($userJoker <= 0) { // We check if the user has enought jokers
